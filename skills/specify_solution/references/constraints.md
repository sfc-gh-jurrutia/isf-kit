...
| Never | Instead |
|-------|--------|
| Run Streamlit locally (`streamlit run`) | Deploy to Snowflake only |
...
| Complex multi-panel apps | React |
| ML training / exploration | Notebook |
| GPU / distributed training | Notebook (Container Runtime) |
| End-user facing (simple) | Streamlit |
| End-user facing (advanced) | React |
...
├── sql/
├── data/
├── streamlit/             # MUST be named streamlit/, NOT app/
│   ├── snowflake.yml
│   ├── environment.yml
│   └── Home.py
├── react/                 # Optional: React+FastAPI copilot
