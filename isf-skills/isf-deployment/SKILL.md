...
- React+FastAPI app in `src/ui/` and `api/` (from `isf-react-app`)
...
| After Deployment | Next Skill |
|-----------------|------------|
| Validate solution works | `isf-testing` |
| Persona review | `isf-persona-reflection` |
| Pre-publication check | `isf-prepublication` |
| Package deliverables | `isf-solution-package` |
