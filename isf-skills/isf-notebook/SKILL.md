...
name: isf-notebook
description: >
  Build Snowflake Notebooks with GPU support and distributed training.
  Use when: (1) creating ML notebooks, (2) configuring compute pools,
...
⚠️ STOP: Confirm notebook section plan (title, objectives, data sources, model approach) before writing cells.
...
Notebooks live in the `notebooks/` directory per the ISF project structure:

```
notebooks/
├── environment.yml          # Conda environment (use .yml not .yaml)
...
| Skill | Relationship |
|-------|-------------|
| `isf-data-architecture` | Notebooks query DATA_MART tables for training data |
| `isf-data-generation` | ML feature store pattern feeds notebook training |
| `isf-python-udf` | Trained models can be deployed as UDFs |
| `isf-solution-style-guide` | Dark theme and colorblind-safe palette for visualizations |
| `isf-deployment` | Deploys notebook to Snowflake compute pool |
